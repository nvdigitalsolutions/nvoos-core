...
class ChatOrchestrator
{
    ...
    public function handleChat(
        ...
        $allowedToolSlugs = $assistantConfig['tools'] ?? [];
        ...
        // Before hook.
        $this->events->dispatch(new BeforeChatRequest(
        ...
        while ($iteration < $maxIterations) {
            ...
            $this->events->dispatch(new AgenticIterationComplete(
                iteration: $iteration,
                assistantId: $assistantId,
            ));
        }
        ...
        // After hook.
        $this->events->dispatch(new AfterChatResponse(
            assistantId: $assistantId,
            response: $response,
            requestContext: ['user_id' => $userId, 'options' => $options],
            durationMs: $durationMs,
        ));

        // Loop completed.
        $this->events->dispatch(new AgenticLoopCompleted(
    ...
    public function handleChatStreaming(
        ...
        if ($this->errors->isError($response)) {
            ...
            $this->sse->sendEvent('error', ['code' => $normalized['code'], 'message' => $normalized['message']]);
            $this->sse->sendDone();
            return ['response' => $normalized, 'tool_results' => [], 'iterations' => 0, 'cost' => null];
        }
        ...
        while ($iteration < $maxIterations) {
            ...
            $this->sse->sendEvent('tool_execution', [
                'type'       => 'start',
                'iteration'  => $iteration,
            ...
            foreach ($toolCalls as $toolCall) {
                ...
                $toolMsg = ['role' => 'tool', 'content' => $resultContent, 'tool_call_id' => $toolCallId];
                if ('' !== $toolName) {
                    $toolMsg['name'] = $toolName;
                }
                $messages[] = $toolMsg;
        ...
        if ('' !== $text) {
            $this->sse->streamChunks($text, function (string $chunk) {
