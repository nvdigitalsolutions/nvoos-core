...
class HuggingFaceDatasetGetRowsTool extends AbstractHuggingFaceTool {
	...
	public function getName(): string {
		return 'HuggingFace Dataset Get Rows'; }
	public function getDescription(): string {
		return 'Retrieves paginated rows from a HuggingFace dataset split.'; }
		...
		try {
			$data = $this->apiGet(
				...
				array(
					...
					'offset'  => $this->intParam( $arguments, 'offset' ),
					'length'  => $this->intParam( $arguments, 'length', 100 ),
				)
			);
			...
			return $this->errors->create( 'hf_rows_failed', $e->getMessage() ); }
	}
}
