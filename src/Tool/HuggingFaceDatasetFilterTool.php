...
class HuggingFaceDatasetFilterTool extends AbstractHuggingFaceTool {
	...
	public function getName(): string {
		return 'HuggingFace Dataset Filter'; }
	public function getDescription(): string {
		return 'Retrieves filtered rows from a dataset split using a where clause.'; }
	public function getParametersSchema(): array {
		return array(
			...
			'properties'           => array(
				...
				'where'   => array(
					'type'        => 'string',
					'description' => 'Filter expression (e.g., "age > 30")',
				),
				'limit'   => array(
					'type'        => 'integer',
	...
	public function execute( array $arguments = array(), array $context = array() ): mixed {
		...
		$where = $this->stringParam( $arguments, 'where' );
		if ( '' === $where ) {
			return $this->errors->validationFailed( 'where is required.', array( 'where' => array( 'Filter expression is required.' ) ) );
		}
		try {
			$data = $this->apiGet(
				'/filter',
				array(
					...
					'where'   => $where,
					'length'  => $this->intParam( $arguments, 'limit', 100 ),
				)
			);
			...
			return $this->errors->create( 'hf_filter_failed', $e->getMessage() ); }
	}
}
