...
class Crawl4AiPriceLookupTool extends AbstractTool {
    ...
    public function getSlug(): string { return 'crawl4ai_price_lookup'; }
    public function getName(): string { return 'Crawl4AI Price Lookup'; }
    public function getDescription(): string { return 'Performs a price/product lookup using Crawl4AI web search.'; }
    public function getParametersSchema(): array { return ['type'=>'object','properties'=>['query'=>['type'=>'string','description'=>'Product search query']],'required'=>['query'],'additionalProperties'=>false]; }
    ...
    public function execute(array $arguments = [], array $context = []): mixed {
        ...
        try {
            $request = new \Nyholm\Psr7\Request('POST',$baseUrl.'/search',['Content-Type'=>'application/json'],json_encode(['query'=>$query]));
        ...
        } catch (\Exception $e) { return $this->errors->create('crawl4ai_failed',"Crawl4AI lookup failed: {$e->getMessage()}"); }
    }
}
